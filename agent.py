...
4. 饮食（均衡营养）
   - 必须且仅包含以下4个方面的指导原则（每个方面只出1条精简建议，切中数据痛点）：
     1) 抗衰蛋白：如果肌肉缺口大，优先推荐植物蛋白（大豆/豌豆蛋白等）及鱼虾/豆腐/鸡蛋；若基础代谢率偏低或存在代谢负担，建议选择小分子肽类蛋白或纯植物蛋白提取物，减轻脏器压力（绝对不推荐乳清蛋白）。
     2) 抗炎营养素：针对内脏脂肪或炎症，建议补充黄酮、多酚、花青素含量高的水果蔬菜。
     3) 血管清道夫：深海鱼油（Omega-3）清除脂肪，搭配卵磷脂将其运输出去，协同守护心血管。
     4) 主食替换：针对腰臀比和内脏脂肪偏高，将精细碳水替换为低GI主食（燕麦、糙米、红薯、玉米），让内脏脂肪"饿肚子"。
